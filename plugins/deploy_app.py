...
def deploy_app(global_plugin_state, current_configuration, output, test_id):
    ...
    run_external_applicaton('kubectl apply -f ./config/deploy/kubernetes/manifest.yaml')
    run_external_applicaton('kubectl apply -f ./config/deploy/kubernetes/monitoring')


def check_status(namespace):
...
def check_pods_status(global_plugin_state, current_configuration, output, test_id):
    with yaspin(Spinners.line, text="Checking pods to be ready. This could take some minutes...") as sp:
        while(True):
            pods_are_running = check_status('sock-shop')
            if(pods_are_running):
